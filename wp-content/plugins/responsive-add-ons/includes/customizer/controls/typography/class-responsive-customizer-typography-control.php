	...
	class Responsive_Customizer_Typography_Control extends WP_Customize_Control {
		...
		public function enqueue() {
			...
			if ( ! class_exists( 'Tribe__Events__Main' ) ) {
				wp_enqueue_script( 'responsive-select2', RESPONSIVE_ADDONS_URI . 'includes/customizer/controls/select2.min.js', array( 'jquery' ), RESPONSIVE_ADDONS_VER, true );
				wp_enqueue_style( 'select2', RESPONSIVE_ADDONS_URI . 'includes/customizer/controls/select2.min.css', RESPONSIVE_ADDONS_VER, true );
				wp_enqueue_script( 'responsive-typography-js', RESPONSIVE_ADDONS_URI . 'includes/customizer/assets/min/js/typography.min.js', array( 'jquery', 'select2' ), RESPONSIVE_ADDONS_VER, true );
			}
			wp_enqueue_style( 'responsive-typography', RESPONSIVE_ADDONS_URI . 'includes/customizer/assets/min/css/typography.min.css', RESPONSIVE_ADDONS_VER, true );

			wp_enqueue_script( 'responsive-typography-weight', RESPONSIVE_ADDONS_URI . 'includes/customizer/controls/typography/typography-weight-control.js', array( 'jquery', 'customize-base' ), RESPONSIVE_ADDONS_VER, true );
			wp_enqueue_script( 'responsive-conditional-display-controls', RESPONSIVE_ADDONS_URI . 'includes/customizer/assets/js/conditional-display-controls.js', array( 'jquery', 'customize-base' ), RESPONSIVE_ADDONS_VER, true );
			wp_localize_script(
				...
				array(
					...
					'custom_fonts' => Responsive_Add_Ons_Custom_Fonts_Taxonomy::get_fonts(),
				)
			);
		}
		...
		public function to_json() {
			...
			$this->json['custom_fonts']    = Responsive_Add_Ons_Custom_Fonts_Taxonomy::get_fonts();
		}
